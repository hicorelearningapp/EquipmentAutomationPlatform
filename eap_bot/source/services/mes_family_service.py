...
class MesFamilyService:
    ...
    def _save_families(self, families: list) -> None:
        try:
    ...
    def _resolve_canonical_family(self, mes_family: str) -> str:
    ...
    def get_mes_families(self) -> list:
        return self._load_families()

    def update_mes_families(self, body: list) -> dict:
        seen_names = set()
        seen_ids = set()
        for item in body:
        ...
        old_families = self._load_families()
        ...
        self._save_families(new_families_list)
    ...
    def get_mes_templates(self, mes_family: str) -> list:
    ...
    def get_mes_template_info(self, mes_family: str, template: str) -> dict:
        family_dir = (MES_MAP_DIR / mes_family).resolve()
        if not family_dir.exists() or not family_dir.is_dir() or MES_MAP_DIR not in family_dir.parents:
            raise HTTPException(404, f"MES Family '{mes_family}' not found")
    ...
    def add_mes_template_info(self, mes_family: str, file_name: str, contents: bytes) -> dict:
        canonical_name = self._resolve_canonical_family(mes_family)
        ...
        template_path = (family_dir / file_name).resolve()
        if template_path.exists():
            raise HTTPException(409, f"Template '{file_name}' already exists in family '{canonical_name}'")
        ...
        try:
            ...
            json_data["TemplateName"] = Path(file_name).stem
            ...
            return {
                "Status": "success",
                "Message": f"Template '{file_name}' added successfully to family '{canonical_name}'"
            }
    ...
    def update_mes_template_info(self, mes_family: str, template: str, file_name: str, contents: bytes) -> dict:
        canonical_name = self._resolve_canonical_family(mes_family)
        family_dir = (MES_MAP_DIR / canonical_name).resolve()
        if not family_dir.exists() or not family_dir.is_dir() or MES_MAP_DIR not in family_dir.parents:
            raise HTTPException(404, f"MES Family '{canonical_name}' not found")
        ...
        try:
            ...
            if "Version" in json_data:
                ...
                json_data["Version"] = self._increment_minor_version(existing_version)
